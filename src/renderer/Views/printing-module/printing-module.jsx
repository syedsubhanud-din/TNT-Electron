...
export default function PrintingModule() {
      ...
      if (window.electron && window.electron.runPython) {
        ...
        try {
          ...
          if (response.status === 'ok') {
            ...
            toast.error('Print engine is already running', {
              id: loadingToast,
            });
          } else {
            toast.error(
              `Print failed: ${response.descript || 'Unknown error'}`,
              { id: loadingToast },
            );
          }
        } catch (e) {
          ...
          if (
            ...
            toast.error(`Print failed: ${output.slice(0, 100)}`, {
              id: loadingToast,
            });
          }
        }
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      const result = await window.electron.invoke('save-template', {
        name: tempTemplateName,
        ...formData,
      });

      console.log('Save Template result', JSON.stringify(result, null, 2));

      if (result.success) {
        toast.success(`Template '${tempTemplateName}' saved successfully!`);
  ...
  return (
    <div className="printing-module-wrapper">
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setPrinterSettings({
                    ...printerSettings,
                    printer_port: parseInt(e.target.value),
                  })
                }
                placeholder="9944"
              />
              ...
              <div className="label-card">
                ...
                <div className="details-section">
                  <div className="detail-row">
                    <strong>GTIN:</strong> <span>{formData.gtin}</span>
                  </div>
                  <div className="detail-row">
                    <strong>MFG:</strong>{' '}
                    <span>{formatDateToMMYYYY(formData.mfgDate)}</span>
                  </div>
                  <div className="detail-row">
                    <strong>EXP:</strong>{' '}
                    <span>{formatDateToMMYYYY(formData.expDate)}</span>
                  </div>
                  <div className="detail-row">
                    <strong>BATCH:</strong> <span>{formData.batch}</span>
                  </div>
                  <div className="detail-row">
                    <strong>SN:</strong> <span>{formData.serialNumber}</span>
                  </div>
                  {formData.tmdaReg && (
                    <div className="detail-row">
                      <strong>TMDA:</strong> <span>{formData.tmdaReg}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="empty-state-placeholder">
              <div className="placeholder-icon">
                ...
                >
                  ...
                  />
                  <path
                    d="M50 10H70V30H50V10Z"
                    stroke="#9ca3af"
                    strokeWidth="4"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M10 50H30V70H10V50Z"
                    stroke="#9ca3af"
                    strokeWidth="4"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M50 50H70V70H50V50Z"
                    stroke="#9ca3af"
                    strokeWidth="4"
                    strokeLinejoin="round"
                  />
              ...
              <p>
                Fill in the label information and generate a QR code to preview
                the label
              </p>
            </div>
          )}
      ...
      <div className="action-bar">
        ...
        <button
          className={`btn-secondary ${!isGenerated ? 'disabled' : ''}`}
          onClick={handlePrint}
        ...
        <button className="btn-outline" onClick={handleSaveTemplate}>
          <svg
            width="18"
            height="18"
            viewBox="0 0 24 24"
          ...
          <div className="modal-content">
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder="e.g. Pharma Standard Label"
                autoFocus
              />
            </div>
